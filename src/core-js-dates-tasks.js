...
function getCountDaysInMonth(month, year) {
  ...
  date.setUTCDate(date.geUTCtDate() - 1);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const period = +Date.parse(dateEnd) - +Date.parse(dateStart);
...
function isDateInPeriod(date, period) {
  ...
  const formatedCurr = new Date(period.start);
  const formatedEnd = new Date(period.end);
  while (formatedCurr <= formatedEnd) {
    if (Date.parse(formatedCurr) === Date.parse(formatedDate)) {
      return true;
    }
    formatedCurr.setDate(formatedCurr.getDate() + 1);
  }
  return false;
}
...
function formatDate(date) {
  ...
  const sec =
    x.getUTCSeconds() >= 10 ? x.getUTCSeconds() : `0${x.getUTCSeconds()}`;
  if (x.getUTCHours() >= 12) {
    postmeridiem = true;
    if (x.getUTCHours() > 12) {
      hours = x.getUTCHours() - 12;
    }
  } else {
    hours = x.getUTCHours();
    postmeridiem = false;
  }
  return `${x.getMonth() + 1}/${x.getUTCDate()}/${x.getUTCFullYear()}, ${hours}:${minutes}:${sec} ${postmeridiem ? 'PM' : 'AM'}`;
}
...
function getWeekNumberByDate(date) {
  const currDate = new Date(date.getFullYear(), 0, 0);
  let dayCounter = 0;
  while (currDate < date) {
    dayCounter += 1;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const current = new Date(reverseDate(period.start));
  const end = new Date(reverseDate(period.end));
  const schedule = [];
  while (current <= end) {
    for (let i = 0; i < countWorkDays; i += 1) {
      schedule.push(reverseDate(current.toISOString().slice(0, 10)));
